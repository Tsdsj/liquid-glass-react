...
> **2026-09-20 更新**：F（工具栏 roving focus）已经**实测复现**，不再是「记在 known-limitations 里但没有用例」。`tests/browser/suspicions.spec.ts` 里有一条用例，断言的是现在的（错误的）行为——方向键按 8 次全部落在按钮上，一次都进不去分段控件。修好之后把那条断言反过来。
