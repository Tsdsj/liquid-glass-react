...
#### 插入：文档站的桌面形态（2026-09-21，第 6 周之后）

所有者看过宽屏效果之后提的，不在原排期里：文档站在 PC 上没有形状。量出来三条——侧栏是 228×114 的胶囊装在 260×1000 的栏里、标题栏和正文错开 130px、目录停在离窗口右边 170px 处——外加示例格只有 327px。

改动分两侧：**库**这边 `TabBar` 的侧边栏形态变成真正的源列表（一整块玻璃跑满栏高，`accessory` 在分隔线下自己滚动），`MenuBarMenu` 补上 `selection`；**站点**这边标题栏和正文对齐成同一列、列宽随窗口分档、示例栅格下限提到 `min(100%, 420px)`，并用 `MenuBar` 搭了一条真的命令栏。`tests/browser/desktop-shell.spec.ts`，12 条。

这**提前吃掉了第 7 周「首页桌面组合演示」的一部分**：站点自己现在就是那个演示，第 7 周要做的从「拼一个窗口出来给人看」变成「把 `Panel` / `PathBar` / 工具栏溢出补进这个已经成形的窗口里」。

三条记在 [`../../../reports/hig-review.md`](../../../reports/hig-review.md)：我凭印象加的行宽上限量完发现更糟、把偏好弹窗当重复控件删错了、以及又一条没有证据的守卫（`flex-wrap: nowrap`）被删掉。
