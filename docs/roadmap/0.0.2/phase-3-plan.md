...
### 已完成（2026-09-20，第 4 周）

`platform?: 'auto' | 'desktop' | 'touch'` 落地，两套度量的对照表在 [`../../api.md`](../../api.md)。计划里那几条的实际做法：

- **度量全部在 CSS 里**，计划原话是「`densityTokens` 加一套 `desktop`」。改成 CSS 是因为 `--lg-control-height` 是写在每个玻璃面上的内联样式：由 JS 决定的话，SSR 页面会先带着触摸度量到达再在 hydrate 时重排，而且内联样式写死之后 `platform="desktop"` 永远赢不了它。`desktopDensityTokens` 仍然导出，给 TypeScript 里需要推理这个数的地方用。
- **命中区**按计划从 44 降到 24，但指针下**只撑竖直方向**：横向撑开的绝对定位区域计入页面可滚动溢出，`layout-matrix.spec.ts` 当场报了五个布局页「RTL 下页面横向滚动 82px」。
- **文字表**按 macOS 那 11 档，两条有意偏离写进了 api.md：11px 下限保留（macOS 把脚注放在 10pt，网页不行）、Dynamic Type 在桌面下照常工作。
- **材质响应**：`--lg-stretch-gain` 一个 token，`pull.ts` 在 `pointerdown` 时读一次。`--lg-press-scale` 按平台分档。计划里还写了「光晕强度」，没做——那一组已经按外观分档过一轮（0.0.2 的深色光晕修复），再按平台分一次需要先有证据。
- **矩阵加了 `desktop` 与 `desktop+dark` 两行**（440 → 520 格），刚加上就逮到两条真问题：代码块 10.1px、分隔线够不着。**对比度用例在两套度量下各跑一次**——4.5:1 在任何字号下都是同一个数（WCAG 只对 18pt/14pt 粗体放宽，这里都不是），但次级灰在桌面表下压在 11px 上，把同一处测量在两套度量下各跑一遍，是一次默认度量的变更不至于悄悄变成一次可读性的变更的办法。

**仍然没做**：圆角不按平台切换。HIG 说 macOS 的小控件是圆角矩形而不是胶囊，而 `radius` 喂给折射几何——改形状和改大小是两个决定。记在这里。
...
### 已完成（2026-09-20，第 3 周）

先收窄清单，再补缺口。清单从 **1255 条读数收到 562 问 2 剩**，四条修正见 [`../../testing.md`](../../testing.md)「动效清单」一节——每一条都是这份清单在骗自己，其中最要命的是探针从来没有滚进视口，于是「没动」常常是「根本没碰到」。

补的：

- **morph 方向**。缩放原点本来就是从触发器算的，问题在那 6px 起手位移不带方向：朝上打开时它往远离按钮的方向长。根因比表面更细——`@starting-style` 是在 `showPopover()` 那一刻读的，而 `data-placement` 是之后才写的。现在开之前先判方向。
- **四个控件当前选中项的悬停**（分段控件、页内标签、标签栏、页码点）：抬的是阴影不是缩放，因为缩放会顶掉正在跑的 transform 过渡。
- **颜色井**：选中环（`box-shadow` 是那条过渡里唯一没列的属性）、井本体的悬停与按压。
- **折叠区标题行、分栏分隔线**的底色过渡。
- **「减少动效」的覆盖面**。计划里写的是「守住反面：这些动画全部归零」，写下这条断言的那一刻它就红了——那条一刀切规则写的是 `.lg-root` 加四个手写的组件名，库里其余部分在这个设置下照常动。这是本周最值钱的一条，而它不在缺口清单里，是反向断言抓出来的。

计划里写着「行按压」，实际**不需要**：清单第一版把列表行报成没有按压态，加上滚动之后它们全部消失——行的底色过渡一直都在，被报出来只是因为探针在视口外。
