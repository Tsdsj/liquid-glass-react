...
#### 已完成（2026-09-21，第 6 周）

`MenuBar`、`GlassMenuItem.alternate`（⌥ 替代项）、`CommandPalette`，文档站的 ⌘K 换成库组件，两个新组件页（44 页）。

计划里给 `MenuBar` 写的三件缺的事全部做了：横向一排、悬停时在已打开的菜单间滑动、⌥ 显示替代项。第三件放进了 `GlassMenu` 而不是 `MenuBar`——macOS 的替代项在任何菜单里都有，写在菜单栏里等于只有菜单栏能用。

`CommandPalette` 按计划付掉了 combobox + listbox 虚拟焦点那套成本，0.4.0 的 `Combobox` 现在缺的只是「输入框本身是值」这一层。

四处值得记下来的：

1. **逮到一个真缺陷，是测试逮的，不是我想到的。** 用鼠标点开一个菜单，光标就停在那个标题上；之后按方向键，浏览器在隐藏一块面板、显示另一块之后会在**原地**补发一个指针事件，照单全收就会把菜单拽回光标底下——按右键，它往右走一格又弹回来，最后一个都不开。修法是比较坐标：合成事件重复的是上一次的位置。
2. **那条用例第一版是假绿的。** 点完立刻按键时，按键赶在浏览器补发的指针事件前面，坏的版本照样过。现在点击和按键之间先断言菜单确实开着——这一行是证据的一部分，不是整洁。
3. **一处守卫撤掉之后没有红，所以删掉了。** 菜单在同一次提交里一开一关，我以为关掉的那个会晚一步报上来把新开的也关掉，于是加了「只有还是它才算数」的判断。撤掉，12 条用例全绿——因为 React 在那一刻已经认为它是关的，那次 `setOpen(false)` 根本不会往外报。和第 1 周的 `grab`、第 5 周的两条守卫是同一件事，只是这次的结论是**删**。顺手补了一条点外面关闭的用例，那条路径此前谁也没测过。
4. **动效清单报出一条新的**：菜单栏标题的按压态只有 90ms 的底色，而库里其他所有可按的东西在手指底下都会动。补了 `transform`，`motion.spec.ts` 多两条（按压会动、减少动效下不动）。

`useModalDialog` 是顺带的：对话框、警告框、面板各写了一遍「记住焦点 → `showModal()` → 锁背景 → 出来时关闭、解锁、还焦」，命令面板会是第四遍。

#### 插入：文档站的 PC 布局（2026-09-21，第 6 周之后）

所有者看过宽屏效果之后提的，不在原排期里，并且**第一版被退回重做**。

第一版按 macOS 应用窗口排：源列表从顶到底，分区和页面都在这一条栏里。组件区勉强成立，概览页给出的是一块 260×1000、里面只有四行字的空玻璃——「整个文档站看起来就像是被压扁了一样」。第二版按主流组件库文档站排：顶部通栏放分区，左列只放当前分区的页面，中间是页面，右边是这一页的目录。

库这边的净改动只剩一条：`TabBar` 的 `sidebarBreakpoint` 现在被当成一条宽度用而不是一个开关——给一个够不着的数，它在所有宽度上保持胶囊形态，由调用方决定放哪。第一版给 `TabBar` 侧边栏形态做的「源列表」改造随第一版一起撤了：站点不再用那个形态，一段没有调用方也没有用例的代码留着只会腐烂。`MenuBarMenu.selection` 留下了，连同它在 `MenuBar` 组件页上的示例和用例——站点的命令菜单撤了，但那个 API 缺口是真的。

`tests/browser/desktop-shell.spec.ts`，11 条，八条修改逐个撤销验证过会红。

这**提前吃掉了第 7 周「首页桌面组合演示」的一部分**，但换了个方向：站点本身不再假装是一个应用窗口，所以第 7 周那个演示仍然要做——它是首页上一块**画出来的**窗口（菜单栏 + 工具栏 + 分栏 + 检查器 + 面板），而不是站点自己的骨架。`MenuBar` 属于那儿。
...
#### 已完成（2026-09-22，第 8 周）

`OutlineView`（48 页），以及按第 4 周那张新度量表把全库重量了一遍。`tests/browser/outline-view.spec.ts`，13 条。

组件这边，HIG 的 outline-views 页给的三条（层级只出现在第一列、Option 点三角形展开整支、记住展开选择）都照做了，多列**明确不做**并写明理由：那是 `treegrid`，做一半对谁都不像表格。三角形不是按钮——`treeitem` 里的按钮是树的键盘模型走不到的焦点位。

一个只有键盘会撞上的缺陷：刚打开的文件夹有 140ms 对键盘和读屏不存在。收起的层用 `content-visibility: hidden`，而**过渡按目标状态那条规则读**，这个离散属性在开合两条规则里都列着，于是两个方向都在动画中途才翻转。现在只有"收起"那条列它。

复审这边量了 48 页 × 三个问题，三条真发现（页码点在光标下只有 18×18、路径栏每一级 22px 高、「…」按钮 31px 宽）。第三条是前两条改完之后矩阵才报出来的——在那之前它两个角恰好落在邻居控件上，被"落到邻居上不算漏"的豁免吸收了。细节在 [`../../../reports/hig-review.md`](../../../reports/hig-review.md)。
...
| ~~`OutlineView`（树）~~ **已完成** | 0.4.0 记为「值得单独一期」；桌面方向下它就是那一期。`role="tree"` 的键盘模型：← → 折叠展开、↑ ↓ 行、Home/End、打字跳转；第一列露层级 | outline-views |
