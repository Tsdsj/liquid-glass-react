# 0.0.3：转向桌面，动效补全，动效可打断

写于 2026-09-20。前一版见 [`../0.0.2/phase-2-plan.md`](../0.0.2/phase-2-plan.md)。
...
版本号按 [`../README.md`](../README.md) 的规则：这里没有一条会让现有代码改——修复、纯新增、新组件、新的可选策略——所以是 0.0.x。0.0.2 至今**未发布**；如果开工时它还没发，这些就并进 0.0.2，号只决定写进更新日志的哪一节。
...
### 守住它的用例（`tests/browser/interruptible.spec.ts`，新）

对分段、标签页、标签栏两种形态、页码点、开关各一条：**滑到一半时给第二次输入，逐帧采样，断言相邻两帧的位移不超过弹簧一帧的最大步长**（连续性），且第一帧不等于上一次的终点。另一条：按下未选中项 6 帧内透镜位置不变。先写、先红、再修。
