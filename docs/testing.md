...
**浏览器（`tests/browser/`，340 项，真实 Google Chrome）**：
...
「该动」是这份清单的全部内容，也是它最初没有的东西。第一版把每一条空读数都算成发现，报出 1255 条，而其中绝大多数是库做对了：焦点环是 `outline`，系统自己的也当场出现；卡片是内容层，不该回应指针；一次什么都没改变的点击没有东西可动；禁用的按钮本来就该无视指针。**一份大部分内容是「正确行为」的报告没有人会读**，而它顶上那个数字比没有数字更糟，因为总会有人引用它。收窄之后是 562 问 2 剩。
