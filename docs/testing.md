...
**浏览器（`tests/browser/`，425 项，真实 Google Chrome）**：
...
| `desktop-parts.spec.ts` | 第 7 周那五样：分组框的标题在框外且绑成一个有名字的分组；路径栏最后一级是位置不是链接、太长时折中间留首尾且折掉的还在菜单里、每一级两种指针下都够得着；浮动面板不模态、方向键能移动、收起真的矮下去、开在容器外时会被夹回来；工具栏从尾部交出项且交出去的仍可达；面板在手机形状的窗口里是边缘面板、在桌面形状的窗口里是居中卡片 |
| `outline-view.spec.ts` | 大纲视图：整棵树一个 Tab 位且位置跟着选中项；↑ ↓ 走看得见的行而不是 DOM（关着的文件夹里那些行一直在 DOM 里）；→ 先开再进、← 先收再退；打字跳转并绕回；**收起就是对所有人收起**（问浏览器 `checkVisibility()` 与能否聚焦，不是问样式表）；收起脚下的文件夹时焦点落在文件夹上而不是掉回 body；三角形开合、名字选中、Option 点开整支；禁用行点不动也按不动；`aria-level` 报层级且层级只体现为缩进；展开集合由调用方保管；打开有高度动画、减少动效下没有；一行两种指针下都够得着；悬停只亮指针底下那一行，不亮它头上的每一层文件夹 |
