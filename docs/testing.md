...
**浏览器（`tests/browser/`，430 项，真实 Google Chrome）**：
...
| `platform.spec.ts` | 桌面度量：有鼠标的宽窗口下标准按钮从 44 降到 36 但不出主流区间、正文仍是**和手指下一样的 17/22**、字号表不跌破 11px 的可读下限、小按钮在两套度量下都够得着（并且真去点一下命中区的像素，不只是读声明）、拖动形变更收敛；`platform` 两个方向都能压过媒体查询；390px 宽即使有鼠标也保持触摸度量；声明触摸时命中区连宽度一起长；分隔线用当前指针的尺寸抓得住；代码块不跟着字号表跌到阅读尺寸以下；按钮标签比正文只低一档 |
| `desktop-parts.spec.ts` | 第 7 周那五样：分组框的标题在框外且绑成一个有名字的分组；路径栏最后一级是位置不是链接、太长时折中间留首尾且折掉的还在菜单里、每一级两种指针下都够得着；浮动面板不模态、方向键能移动、收起真的矮下去、开在容器外时会被夹回来；工具栏从尾部交出项且交出去的仍可达；面板在手机形状的窗口里是边缘面板、在桌面形状的窗口里是居中卡片；浮动面板里的按钮用的是面板自己的那层玻璃，不是第二层 |
