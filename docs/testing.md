...
**浏览器（`tests/browser/`，396 项，真实 Google Chrome）**：
...
| `platform.spec.ts` | 桌面度量：有鼠标的宽窗口下标准按钮是 22 不是 44、正文 13/16、字号表不跌破 11px 的可读下限、22px 的按钮仍然有 24px 的命中区（WCAG 2.2 指针下限）、拖动形变更收敛；`platform` 两个方向都能压过媒体查询；390px 宽即使有鼠标也保持触摸度量；分隔线用当前指针的尺寸抓得住；代码块不跟着字号表跌到 12px 以下 |
...
「该动」是这份清单的全部内容，也是它最初没有的东西。第一版把每一条空读数都算成发现，报出 1255 条，而其中绝大多数是库做对了：焦点环是 `outline`，系统自己的也当场出现；卡片是内容层，不该回应指针；一次什么都没改变的点击没有东西可动；禁用的按钮本来就该无视指针。**一份大部分内容是「正确行为」的报告没有人会读**，而它顶上那个数字比没有数字更糟，因为总会有人引用它。收窄之后是 562 问 2 剩；44 页时是 2712 探针、616 问、3 剩，三条都是已经记下来的决定。
