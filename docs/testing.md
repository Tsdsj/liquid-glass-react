...
**浏览器（`tests/browser/`，421 项，真实 Google Chrome）**：
...
| `command-palette.spec.ts` | 焦点一直在输入框里而高亮由 `aria-activedescendant` 点名、打字筛选后高亮落在还存在的那一行、回车执行、不可用的命令画得出来但跳得过也点不动（且那一下不会把焦点带走）、没有匹配时说的是应用自己的话、Escape 关闭并还焦、关掉不留输入、页面印的那个修饰键才是真的绑定（同一组键从面板里面按是关、从外面按是开）、`shortcut={null}` 什么都不绑、`filter={false}` 照给的顺序画、焦点环只有一层且画在字段那一行上 |
