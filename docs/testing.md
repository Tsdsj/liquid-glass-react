...
**浏览器（`tests/browser/`，390 项，真实 Google Chrome）**：
...
| `desktop-shell.spec.ts` | 文档站在宽屏上是一个窗口：标题栏和正文是同一列、菜单栏前缘对齐标题前缘、侧栏是跑满栏高的源列表且页面列表滚动时分区不动、目录贴着正文列的后缘、正文有行宽上限而示例没有且示例不小于 420、菜单项真的改变窗口（文字大小与减少动效落到 `<html>` 上）、外观是单选而辅助功能是复选；窄屏没有菜单栏、一列一个示例、不横向溢出 |
