...
| `size` | `'small' \| 'large'` | `'small'` | 大玻璃更厚、阴影更深、**不翻转**明暗——但 `clear` 不在此列，它的定义就是能看穿，压暗层仍是 35%。 |
...
**面板里的控件用的是面板自己的那层玻璃。** 和对话框、气泡、卡片形态的面板一样，正文是一块 `SharedSurface`：里面的按钮、分段控件不会再长出第二层玻璃。这一条是补上的——在此之前 `Panel` 是唯一一个不这么做的玻璃容器，而它唯一的用例里装的是滑块，滑块的轨道本来就是内容层，所以一直没有人撞上。
