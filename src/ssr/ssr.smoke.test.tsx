...
const CASES: Array<[name: string, element: React.ReactElement]> = [
  ...
  ['Dropdown (closed)', <Dropdown label="更多" items={[{ key: 'a', label: '编辑' }]} />],
  ['InputNumber', <InputNumber aria-label="数量" defaultValue={3} min={0} max={10} />],
  ['Rate', <Rate aria-label="评分" defaultValue={3} />],
  ['RangePicker (closed)', <RangePicker aria-label="日期范围" />],
  ['TimePicker (closed)', <TimePicker aria-label="时间" value="09:30" />],
  ['Timeline', <Timeline items={[{ key: 'a', content: '创建', time: '09:00' }]} />],
  ['Upload', <Upload aria-label="附件" defaultFileList={[{ key: 'k', name: 'a.txt', status: 'done' }]} />],
  [
    'Drawer (closed)',
