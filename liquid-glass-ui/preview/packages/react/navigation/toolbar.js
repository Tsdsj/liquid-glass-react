...
import { GlassSurface, SharedSurface } from '../system/surface.js';
 ...
 */
export const GlassToolbar = forwardRef(function GlassToolbar({ className, children, orientation = 'horizontal', onKeyDown, onFocusCapture, ...props }, ref) {
    ...
    return _jsx("div", { ...props, ref: merged, role: "toolbar", "aria-orientation": orientation, className: cx('lg-toolbar', className), "data-orientation": orientation, onKeyDown: keyboard, onFocusCapture: event => { onFocusCapture?.(event); if (event.target.tagName === 'BUTTON')
            setTabStop(event.target); }, children: children });
});
/**
 ...
 */
export const ToolbarGroup = forwardRef(function ToolbarGroup({ className, children, prominent = false, radius = 'pill', ...props }, ref) {
    const [root, merged] = useMergedRef(ref);
    ...
    return _jsx(GlassSurface, { ...props, ref: merged, radius: radius, "data-prominent": prominent ? 'true' : undefined, className: cx('lg-toolbar-group', className), children: _jsxs(SharedSurface, { value: true, children: [fusion, children] }) });
});
/**
 ...
 */
export function ToolbarSpacer({ variant = 'fixed' }) {
    return _jsx("span", { className: "lg-toolbar-spacer", "data-variant": variant, "aria-hidden": "true" });
}
