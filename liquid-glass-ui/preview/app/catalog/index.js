...
export const groupOrder = ['内容层', '控件', '输入', '导航', '浮层'];
...
export const findDoc = (slug) => componentDocs.find(doc => doc.slug === slug);
/** Substring match over name, slug and summary — enough for a catalogue this size. */
export function searchDocs(query) {
    const needle = query.trim().toLocaleLowerCase();
    if (!needle)
        return [];
    return componentDocs.filter(doc => doc.name.toLocaleLowerCase().includes(needle)
