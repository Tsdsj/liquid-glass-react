...
export default defineConfig({
  ...
  projects: [
    { name: 'chrome', use: { ...devices['Desktop Chrome'], channel: 'chrome' }, testIgnore: /(fallback|warnings|strict-mode|hydration)\.spec\.ts/ },
    { name: 'chromium', use: { ...devices['Desktop Chrome'], ...(process.env.CHROMIUM_PATH ? { launchOptions: { executablePath: process.env.CHROMIUM_PATH } } : {}) }, testIgnore: /(fallback|warnings|strict-mode|hydration)\.spec\.ts/ },
