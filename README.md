...
| **浮层** | GlassPopover、GlassMenu、GlassMenuButton、ContextMenu、Tooltip、GlassSheet、GlassAlert、GlassActionSheet、GlassDialog、ToastProvider、Banner |
...
- **每个组件都交还它渲染的那个元素**，并且透传 `id`、`style`、`data-*`、`aria-*`。开发模式下还会对几条最容易踩的设计错误出声——玻璃套玻璃、一屏两个主操作、`clear` 用在没声明色调的地方——生产构建里这些完全不存在。
...
```text
70 单元 · 3 SSR · 103 真实 Google Chrome · 10 开发模式 · 18 WebKit + Firefox
```
